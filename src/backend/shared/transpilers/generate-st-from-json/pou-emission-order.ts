...
function extractPouDeps(
  ...
  if (
    ...
    // xmlBody-only graphical case: SFC and FBD/LD lacking ldBody.
    // The DOM walker the driver falls back to handles its own
    // recursion via `GeneratePouProgram` in the inner generator —
    // this pre-pass only orders POUs whose deps we can see from
    // the IR.  Missing deps surface here as out-of-order emission,
    // not as missing emission, which is acceptable while the
    // ldBody/SFC IR coverage gap closes.
  } else if (
