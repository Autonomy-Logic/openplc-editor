 ...
 *
 * Blob layout (106 bytes, packed, no padding):
 *
 ...
 *   | 22     | productId   | uint8[8]      | 8    |
 *   | 30     | issuedAt    | uint32 LE     | 4    |  unix time
 *   | 34     | expiresAt   | uint32 LE     | 4    |  0 = perpetual
 *   | 38     | signature   | uint8[64]     | 64   |  ECDSA P-256 r||s raw
 *   | 102    | crc32       | uint32 LE     | 4    |  CRC-32/ISO-HDLC over [payload||signature] (0..101)
 *   | 106    | (end)       |               |      |
 *
 * The signed payload is offsets 0..37 (38 bytes). The crc32 covers
 * `[payload || signature]` = offsets 0..101 (102 bytes) and never itself.
 */

/** Total blob size in bytes (`sizeof(lic_blob_t)`). */
export const LIC_BLOB_SIZE = 106
/** Signed payload size in bytes (`sizeof(lic_payload_t)`). */
export const LIC_PAYLOAD_SIZE = 38
/**
...
const OFF_ISSUED_AT = 30
const OFF_EXPIRES_AT = 34
const OFF_SIGNATURE = 38
const OFF_CRC32 = 102
...
export interface LicenseBlob {
  ...
  deviceId: Uint8Array
  /** 8-byte product identifier. */
  productId: Uint8Array
  /** unix time of issuance. */
  issuedAt: number
  /** unix time of expiry; 0 = perpetual. */
  expiresAt: number
...
/**
 * Serialize a `LicenseBlob` into its 106-byte on-wire representation.
 *
 ...
 * (offsets 0..101) — the `crc32` field on the input is ignored.
 */
export function serializeLicenseBlob(b: LicenseBlob): Uint8Array {
  ...
  out[OFF_FLAGS] = b.flags & 0xff
  copyFixed(out, OFF_DEVICE_ID, b.deviceId, DEVICE_ID_SIZE)
  copyFixed(out, OFF_PRODUCT_ID, b.productId, PRODUCT_ID_SIZE)
  view.setUint32(OFF_ISSUED_AT, b.issuedAt >>> 0, true)
  view.setUint32(OFF_EXPIRES_AT, b.expiresAt >>> 0, true)
  copyFixed(out, OFF_SIGNATURE, b.signature, SIGNATURE_SIZE)

  // Recompute crc32 over [payload || signature] = offsets 0..101 (102 bytes).
...
/**
 * Deserialize a 106-byte blob into a `LicenseBlob`. All multi-byte fields are
...
export function deserializeLicenseBlob(buf: Uint8Array): LicenseBlob {
  ...
  return {
    ...
    issuedAt: view.getUint32(OFF_ISSUED_AT, true),
    expiresAt: view.getUint32(OFF_EXPIRES_AT, true),
