/**
 * License-identity derivation helpers (PLA-05, D17/D21/D54/D57).
 *
 ...
 * process during the post-flash activation routine, so they use
...
/** Domain-separation prefix for the device-id digest (D17/D57). Mixed
...
/** VPP id = first 8 bytes of the digest (matches `lic_blob_t.productId`, D54). */
...
export function deriveDeviceId(anchor: Uint8Array): string {
  ...
  return createHash('sha256').update(input).digest('hex').slice(0, DEVICE_ID_BYTES * 2)
}
 ...
 * (D54) so the firmware can bind a written license to its VPP.
 */
export function deriveVppId(packageId: string): string {
  // Hash the package id as its UTF-8 (== ASCII) bytes.
  return createHash('sha256').update(packageId).digest('hex').slice(0, VPP_ID_BYTES * 2)
}
