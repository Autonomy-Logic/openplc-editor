...
// Golden 106-byte blob hex from on-device-license-storage's license-golden.json
// (expectedBytesHex). The mock must emit exactly these bytes.
const GOLDEN_HEX =
  '4f504c430100000102030405060708090a0b0c0d0e0fa0a1a2a3a4a5a6a700105e5f004c5d6a11111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111a78d94c0'
...
describe('checkDeviceActivation (mock toggle)', () => {
  ...
  it('returns licensed + a 106-byte golden blob when OPLC_LICENSE_MOCK=licensed', async () => {
    process.env.OPLC_LICENSE_MOCK = 'licensed'
    const result = await checkDeviceActivation(INPUT)
    expect(result.licensed).toBe(true)
    expect(result.license).toHaveLength(106)
