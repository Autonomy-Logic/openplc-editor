...
export type ToolbarProps = {
  ...
  setPosition: Dispatch<SetStateAction<ToolbarPositionProps>>
  title?: string
...
const Toolbar: FC<ToolbarProps> = ({
  ...
  useEffect(() => {
    setPosition((state) => ({
      x:
        state.x + (nodeRef?.current?.offsetWidth || 0) > width
          ? width - (nodeRef?.current?.offsetWidth || 0)
          : state.x,
      y:
        state.y + (nodeRef?.current?.offsetHeight || 0) > height
  ...
  const Divider = () => {
    return (
      <div className="h-full w-[0.125rem] rounded bg-gray-200 shadow-inner dark:bg-gray-700" />
    )
  }
    ...
    >
      <div
        ...
        className={classNames(
          'flex gap-3 bg-white p-2 dark:bg-gray-900',
          isFloatingOnScreen ? 'h-14 shadow' : 'h-full',
          isCurrentToolbar ? 'z-40' : 'z-30',
        )}
      >
        {tools.map(
          ({ id, onClick, icon: Icon, className, tooltip, divider, type }) => (
            <Fragment key={id}>
              {type === 'toggleTheme' && (
                <Tooltip label={t('menuToolbar.toggleTheme')}>
                  <Toggle
                    ...
                    icons={{
                      enabled: (
                        <HiMoon className="h-3 w-3 text-open-plc-blue" />
                      ),
        ...
        {isFloatingOnScreen && (
          <div className="handle absolute -top-5 left-0 flex h-5 w-full cursor-move items-center justify-center bg-gray-400 dark:bg-gray-800">
            ...
            <Tooltip label={t('common.close')}>
              ...
              >
                <HiXMark className="mr-1 h-4 w-4 text-gray-800 dark:text-gray-500" />
              </button>
            </Tooltip>
          </div>
        )}
