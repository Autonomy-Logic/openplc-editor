 ...
 * prod configs) exactly like `VPP_CATALOG_URL`. Electron renderer bundles have
 * no live `process.env`, so this is evaluated when the bundle is built: set it
 * in the shell BEFORE `npm run dev` to aim the buy flow at a local Edge app.
 *
 *     OPENPLC_EDGE_WEB_URL=http://localhost:5173 npm run dev
 *
 * The release pipeline doesn't set it; webpack's empty-string default is falsy,
 * so shipped builds always point at production.
 */
