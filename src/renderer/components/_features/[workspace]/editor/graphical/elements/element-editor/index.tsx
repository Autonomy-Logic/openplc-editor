...
type ModifierOption = { label: string; contact: string }
...
const ElementEditor = ({ title, triggerLabel, modifierOptions }: ElementEditorProps) => {
  ...
  return (
    <Modal>
      <ModalTrigger>{triggerLabel}</ModalTrigger>
      <ModalContent
        onClose={handleCloseModal}
        className='h-[498px] w-[468px] select-none flex-col justify-between px-8 py-4'
      >
        <ModalTitle className='text-xl font-medium text-neutral-950 dark:text-white'>{title}</ModalTitle>
              ...
              {modifierOptions.map((modifier, index) => (
                ...
                >
                  <input
                    type='radio'
                    name='modifier'
                    className={`border-1 h-4 cursor-pointer w-4 appearance-none rounded-full border border-[#D1D5DB] ring-0 checked:border-[5px] checked:border-brand dark:border-neutral-850 dark:bg-neutral-300`}
                    id={modifier.label}
