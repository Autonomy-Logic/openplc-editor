...
export default function Viewer({ children }: RecentViewerProps): ReactNode {
  return (
    <ScrollArea.Root className='w-full h-[600px] overflow-hidden rounded bg-none pb-2'>
      <ScrollArea.Viewport className='w-full h-full container mx-auto'>
        <div className='w-full h-full grid grid-cols-4 pr-3 gap-4'>{children}</div>
      </ScrollArea.Viewport>
