...
import { PouHistory, PouHistorySnapshot, ProjectResponse, ProjectSlice } from './types'
...
const createProjectSlice: StateCreator<ProjectSlice, [], [], ProjectSlice> = (setState, getState) => ({
  ...
  projectActions: {
    ...
    pushToHistory: (pouName) => {
      setState(
        produce((state: ProjectSlice & { undoRedo: Record<string, PouHistory> }) => {
          ...
          const pou = state.project.data.pous.find((pou) => pou.data.name === pouName)
          ...
          const snapshot: PouHistorySnapshot = {
            variables: [],
            body: undefined,
          ...
          if (pou) {
            snapshot.variables = JSON.parse(JSON.stringify(pou.data.variables))
            snapshot.body = JSON.parse(JSON.stringify(pou.data.body))
          }

          if (flow) {
            snapshot.ladderFlow = JSON.parse(JSON.stringify(flow))
          }

          if (fbdFlow) {
            snapshot.fbdFlow = JSON.parse(JSON.stringify(fbdFlow))
          }

          if (globalVariables.length) {
            snapshot.globalVariables = JSON.parse(JSON.stringify(globalVariables))
          }
          ...
          if (instances.length) {
            snapshot.instances = JSON.parse(JSON.stringify(instances))
          }
          ...
          if (isDataType) {
            ...
            snapshot.dataTypes = JSON.parse(JSON.stringify(dataType))
          }

          state.undoRedo[pouName].past.push(snapshot)

          if (state.undoRedo[pouName].past.length > 50) {
            state.undoRedo[pouName].past.shift()
          }

          state.undoRedo[pouName].future = []
        }),
      )
    },
    ...
    undo: (pouName) => {
      setState(
        produce((state: ProjectSlice & { undoRedo: Record<string, PouHistory> }) => {
          ...
          const pou = isResource ? undefined : state.project.data.pous.find((p) => p.data.name === pouName)
          ...
          const currentSnapshot: PouHistorySnapshot = {
            variables: pou ? JSON.parse(JSON.stringify(pou.data.variables)) : [],
            body: pou ? JSON.parse(JSON.stringify(pou.data.body)) : undefined,
            ladderFlow: flowIndex !== -1 ? JSON.parse(JSON.stringify(ladderFlows[flowIndex])) : undefined,
            fbdFlow: fbdIndex !== -1 ? JSON.parse(JSON.stringify(fbdFlows[fbdIndex])) : undefined,
            globalVariables: globalVariables.length ? JSON.parse(JSON.stringify(globalVariables)) : [],
            tasks: tasks.length ? JSON.parse(JSON.stringify(tasks)) : [],
            instances: instances.length ? JSON.parse(JSON.stringify(instances)) : [],
            dataTypes: isDataType ? JSON.parse(JSON.stringify(dataTypes.find((dt) => dt.name === pouName))) : undefined,
          }
          ...
          if (isResource) {
            state.project.data.configuration.resource.globalVariables = previous.globalVariables as PLCGlobalVariable[]
            state.project.data.configuration.resource.tasks = previous.tasks as PLCTask[]
            state.project.data.configuration.resource.instances = previous.instances as PLCInstance[]
            return
          }

          if (isDataType && previous.dataTypes) {
            const idx = state.project.data.dataTypes.findIndex((dt) => dt.name === pouName)
            if (idx !== -1) {
              state.project.data.dataTypes[idx] = previous.dataTypes as unknown as PLCDataType
            }
            return
          }
          ...
          if (previous.ladderFlow) {
            ...
            state.ladderFlows = ladderFlows.filter((ladderFlow) => ladderFlow.name !== pouName)
          }

          if (previous.fbdFlow) {
            if (fbdIndex !== -1) {
              state.fbdFlows = [...fbdFlows.slice(0, fbdIndex), previous.fbdFlow, ...fbdFlows.slice(fbdIndex + 1)]
            } else {
              state.fbdFlows = [...fbdFlows, previous.fbdFlow]
            }
          } else {
            state.fbdFlows = fbdFlows.filter((f) => f.name !== pouName)
          }
        }),
      )
    },
    ...
    redo: (pouName) => {
      setState(
        produce((state: ProjectSlice & { undoRedo: Record<string, PouHistory> }) => {
          ...
          if (isResource) {
            state.project.data.configuration.resource.globalVariables = next.globalVariables as PLCGlobalVariable[]
            state.project.data.configuration.resource.tasks = next.tasks as PLCTask[]
            state.project.data.configuration.resource.instances = next.instances as PLCInstance[]
            return
          }
          ...
          if (isDataType && next.dataTypes) {
            const idx = state.project.data.dataTypes.findIndex((dt) => dt.name === pouName)
            if (idx !== -1) {
              state.project.data.dataTypes[idx] = next.dataTypes as unknown as PLCDataType
            }
            return
          }
          ...
          if (next.ladderFlow) {
            ...
            if (index !== -1) {
              state.ladderFlows = [...ladderFlows.slice(0, index), next.ladderFlow, ...ladderFlows.slice(index + 1)]
            } else {
              state.ladderFlows = [...ladderFlows, next.ladderFlow]
            }
          } else {
            state.ladderFlows = ladderFlows.filter((ladderFlow) => ladderFlow.name !== pouName)
          }
          ...
          if (next.fbdFlow) {
            if (fbdIndex !== -1) {
              state.fbdFlows = [...fbdFlows.slice(0, fbdIndex), next.fbdFlow, ...fbdFlows.slice(fbdIndex + 1)]
            } else {
              state.fbdFlows = [...fbdFlows, next.fbdFlow]
            }
          } else {
            state.fbdFlows = fbdFlows.filter((f) => f.name !== pouName)
          }
