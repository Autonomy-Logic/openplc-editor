...
describe('findLastEndVarIndex', () => {
  ...
  it('stops at the end of the declaration, not at an END_VAR in the body', () => {
    ...
    // wrote a moment earlier stops parsing. `stopAtIndex` is what keeps the
    // scan out of the body, so it is passed the way the callers pass it.
    const content = [
      'VAR',
      '  FB0 : BLOCK;',
      'END_VAR',
    ...
    const bodyStart = findGraphicalBodyStartIndex(content, 0)
    const idx = findLastEndVarIndex(content, content.indexOf('VAR'), bodyStart)
