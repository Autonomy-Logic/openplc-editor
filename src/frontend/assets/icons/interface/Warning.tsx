...
/**
 * The path is filled, not stroked (`stroke='none'` below), so these were
 * `stroke-*` classes painting nothing: every caller got the hard-coded blue
 * whatever they asked for. Three places in the app try to colour this icon
 * (`stroke-amber-500`, `text-amber-500`, and the licence modal's severity) and
 * none of them worked. `default` is `text-brand` rather than `text-brand-light`
 * because `--primary-default` IS the #0464fb the path used to hard-code, so
 * every existing caller keeps the colour it renders today.
 */
const variantClasses = {
  default: 'text-brand',
  primary: 'text-brand',
  secondary: 'text-brand-medium-dark',
}
