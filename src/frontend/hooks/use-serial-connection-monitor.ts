/**
 * useSerialConnectionMonitor — mirrors the held baremetal serial link into the
 ...
 * all depend on `serialConnection.status` being true. Subscribing inside the
...
export const useSerialConnectionMonitor = (): void => {
  const device = useDevice()
  const setSerialConnectionStatus = useOpenPLCStore((state) => state.deviceActions.setSerialConnectionStatus)
  const clearDeviceProbe = useOpenPLCStore((state) => state.deviceActions.clearDeviceProbe)
  const openModal = useOpenPLCStore((state) => state.modalActions.openModal)

  useEffect(() => {
    return device.onConnectionStatus(({ status, port, reason }) => {
      setSerialConnectionStatus(status, port)
      ...
      if (status === 'error' && reason === 'lost') {
        openModal('runtime-connection-lost', {
          label: port ?? 'the device',
          body: `The connection to ${port ?? 'the device'} was lost and could not be restored. Check that the cable is plugged in and the port is not in use, then Connect again.`,
        })
      }
    })
  }, [device, setSerialConnectionStatus, clearDeviceProbe, openModal])
}
