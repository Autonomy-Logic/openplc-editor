...
function detectParallelRegions(
  ...
  for (let i = 0; i < branch.nodeIds.length; i++) {
    ...
    for (const sn of serial) {
      const style = getDefaultNodeStyle({ node: sn })
      if (style.height > maxSerialHeight) {
        maxSerialHeight = style.height
...
export function addRailBranchHandle(
  ...
  const railId = direction === 'input' ? 'left-rail' : 'right-rail'
...
export function updateRailForBranches(nodes: Node[], handleBranches: HandleBranch[] | undefined): Node[] {
  ...
  for (const branch of handleBranches) {
    ...
    const railId = branch.direction === 'input' ? 'left-rail' : 'right-rail'
...
export function insertIntoBranch(
  ...
  if (isInput) {
    ...
    sourceId = idx === 0 ? 'left-rail' : nodeIds[idx - 1]
    ...
    targetId = idx === nodeIds.length ? 'right-rail' : nodeIds[idx]
...
export function replaceVariableWithBranch(
  ...
  if (isInput) {
    ...
    const edge1 = buildEdge('left-rail', newElement.id, {
    ...
    const edge2 = buildEdge(newElement.id, 'right-rail', {
      sourceHandle: 'output',
...
export function startParallelInBranch(
  ...
  const { edgesToRemove, edgesToAdd } = wireParallelAroundElement({
    incomingEdge,
    outgoingEdge,
    openParallel: openParallel as Node,
    closeParallel: closeParallel as Node,
    aboveElement,
    newElement,
  })
...
export function reconcileBranches(
  ...
  for (const branch of branchesToKeep) {
    ...
    const railId = branch.direction === 'input' ? 'left-rail' : 'right-rail'
    const rail = newNodes.find((n) => n.id === railId)
...
export function reconcileBranchNodeIds(rung: RungLadderState, branch: HandleBranch): string[] {
  ...
  let currentId = isInput ? 'left-rail' : branch.blockId
  ...
  while (true) {
    ...
    const endpointId = isInput ? branch.blockId : 'right-rail'
