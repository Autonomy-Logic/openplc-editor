...
import { type Diagnostic, type MessageConnection, PublishDiagnosticsNotification } from 'vscode-languageserver-protocol'
...
export function attachDiagnosticsBridge(connection: MessageConnection, monacoApi: typeof monaco): DiagnosticsBridge {
  const subscription = connection.onNotification(PublishDiagnosticsNotification.type, (params) => {
    ...
    if (parsed) {
      const varsModel = monacoApi.editor.getModels().find((m) => m.uri.toString() === pouVarsUri(parsed.name))
      if (varsModel) {
