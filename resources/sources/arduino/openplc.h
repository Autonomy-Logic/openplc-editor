 ...
 *
 * Two sets, picked by MCU family, and both are DEFAULTS rather than fixed
 * facts: `io_sizes.h` is generated by the editor when a project asks for more
 * room than the family default, and every macro below is `#ifndef`-guarded so
 * that file wins.
 *
 * `io_sizes.h` has to be reachable from HERE rather than from `defines.h`,
 * even though `defines.h` is where the rest of the generated configuration
 * lives. This header is included by ~20 HAL translation units that never see
 * `defines.h`, and `defines.h` deliberately carries no include guard so it can
 * only reach a TU through exactly one path (see modbus_config.h). A guarded
 * file of its own is the only way to reach every consumer without breaking
 * that rule.
 *
 * These sizes are not only Modbus buffer sizes: they also dimension the IEC
 * pointer arrays below, and `mapEmptyBuffers()` aliases %MW/%MD/%ML straight
 * into the Modbus banks. Changing one without the others silently mismaps I/O,
 * which is why they are generated together or not at all.
 */
/*********************/
#if defined(__has_include)
    #if __has_include("io_sizes.h")
        #include "io_sizes.h"
    #endif
#endif
