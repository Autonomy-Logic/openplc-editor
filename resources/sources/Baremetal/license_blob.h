...
of the struct (magic, timestamps, crc32). This is INDEPENDENT of the Modbus wire,
which carries the transfer `len` in BIG-ENDIAN (see modbus_pdu.cpp / modbus_debug.cpp).
...
//  22   product_id    uint8_t[8]     8
//  30   issued_at     uint32_t LE    4    unix time
//  34   expires_at    uint32_t LE    4    0 = perpetual
//  38   (end of signed payload — payload = 38 bytes)
//  38   signature     uint8_t[64]   64    ECDSA P-256 r||s raw (not DER)
// 102   crc32         uint32_t LE    4    over [payload||signature] (offsets 0..101)
// 106   (end of blob — sizeof(lic_blob_t) == 106)
...
typedef struct {
    ...
    uint32_t issued_at;
    uint32_t expires_at;     // 0 = perpetual
} lic_payload_t;             // 38 bytes

typedef struct __attribute__((packed)) {
    lic_payload_t payload;   // offsets 0..37
    uint8_t  signature[64];  // offsets 38..101 (ECDSA P-256 r||s raw)
    uint32_t crc32;          // offset 102 (covers [payload||signature], 0..101)
} lic_blob_t;                // 106 bytes
#pragma pack(pop)
...
// Bitwise loop (no 256-entry table) to save flash on AVR — the blob is only 102
